...
public class RadiusClusterAwareScheduledTaskRunnerTest extends AbstractRadiusTest {
    ...
    public void beforeMethod() {
        ...
        reset(provider);
        when(provider.executeIfNotExecuted(anyString(), anyInt(), any())).thenReturn(ExecutionResult.executed("executed"));
    }


    @Test
    public void testClusterEvent() throws InterruptedException {
        RadiusClusterAwareScheduledTaskRunner clusterAwareScheduledTaskRunner = new RadiusClusterAwareScheduledTaskRunner(session.getKeycloakSessionFactory(), new ScheduledTask() {
            @Override
    ...
    public void testClusterEventNotExecuted() throws InterruptedException {
        reset(provider);
        when(provider.executeIfNotExecuted(anyString(), anyInt(), any())).thenReturn(ExecutionResult.notExecuted());
        RadiusClusterAwareScheduledTaskRunner clusterAwareScheduledTaskRunner = new RadiusClusterAwareScheduledTaskRunner(session.getKeycloakSessionFactory(), new ScheduledTask() {
            @Override
            public void run(KeycloakSession session) {
            }
    ...
    public void testClusterEventNotExecuted2() throws InterruptedException {
        ...
        when(provider.executeIfNotExecuted(anyString(), anyInt(), any())).then(new Answer<Object>() {
            @Override
