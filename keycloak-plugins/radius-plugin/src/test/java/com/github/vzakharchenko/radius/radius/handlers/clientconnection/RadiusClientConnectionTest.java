...
public class RadiusClientConnectionTest extends AbstractRadiusTest {
    ...
    public void testRadiusClientConnection() {
        InetSocketAddress inetSocketAddress =
                new InetSocketAddress(ModelBuilder.IP, 0);
        AccessRequest accessRequest =
                new AccessRequest(realDictionary, 0, new byte[16]);
        RadiusClientConnection radiusClientConnection =
                new RadiusClientConnection(inetSocketAddress,
                        accessRequest);
        assertEquals(radiusClientConnection
                .getLocalAddr(), "123.123.123.123");
        assertEquals(radiusClientConnection
                .getLocalPort(), 0);
        assertNotNull(radiusClientConnection
                .getRemoteAddr());
        assertNotNull(radiusClientConnection
                .getInetSocketAddress());
        assertEquals(radiusClientConnection
                .getRemoteHost(), "123.123.123.123");
        assertEquals(radiusClientConnection.getRemotePort(),
                0);
        accessRequest.addAttribute("Calling-Station-Id",
                "192.168.1.1");
        accessRequest.addAttribute("NAS-IP-Address",
                "192.168.1.1");
    ...
    public void testRadiusClientConnection2() {
        InetSocketAddress inetSocketAddress = InetSocketAddress
                .createUnresolved(ModelBuilder.IP, 0);
        AccessRequest accessRequest =
                new AccessRequest(realDictionary, 0, new byte[16]);
        RadiusClientConnection radiusClientConnection =
                new RadiusClientConnection(inetSocketAddress,
