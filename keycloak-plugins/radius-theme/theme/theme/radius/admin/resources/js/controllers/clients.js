...
module.controller('ClientDetailCtrl', function($scope, realm, client, flows, $route, serverInfo, Client, ClientDescriptionConverter, Components, ClientStorageOperations, $location, $modal, Dialog, Notifications, TimeUnit2) {
    ...
    function updateProperties() {
        ...
        $scope.radiusOTP = $scope.client.attributes['radius.OTP'] === undefined ||  $scope.client.attributes['radius.OTP'] === null || $scope.client.attributes['radius.OTP'] === 'true';
    ...
    $scope.changeProtocol = function() {
        ...
        }else if ($scope.protocol == "radius-protocol") {
            $scope.clientEdit.protocol = "radius-protocol";
        }
    };
    ...
    $scope.changeOTPCode = function($event) {
            $event.preventDefault();
            $scope.radiusOTP = !$scope.radiusOTP;
            $scope.clientEdit.attributes['radius.OTP'] = $scope.radiusOTP;
    }
    ...
    $scope.save = function() {
        ...
        if ($scope.clientEdit.protocol != 'saml' && $scope.clientEdit.protocol != 'radius-protocol' && !$scope.clientEdit.bearerOnly && ($scope.clientEdit.standardFlowEnabled || $scope.clientEdit.implicitFlowEnabled) && (!$scope.clientEdit.redirectUris || $scope.clientEdit.redirectUris.length == 0)) {
...
module.controller('CreateClientCtrl', function($scope, realm, client, $route, serverInfo, Client, ClientDescriptionConverter, $location, $modal, Dialog, Notifications) {
    ...
    $scope.changeProtocol = function() {
        if ($scope.protocol == "openid-connect") {
            ...
            $scope.client.protocol = "radius-protocol";
        }
    };
...
module.controller('ClientScopeDetailCtrl', function($scope, realm, clientScope, $route, serverInfo, ClientScope, $location, $modal, Dialog, Notifications) {
    ...
    $scope.changeProtocol = function() {
        if ($scope.protocol == "openid-connect") {
            ...
            $scope.clientScope.protocol = "radius-protocol";
        }
    };
