...
public class RadiusComponentProxyTest extends AbstractRadiusTest {
    ...
    public void beforeMethods() {
        ...
        //
        when(provider.getRadiusEndpoint(session, AccessRequest.class)).thenReturn(radiusEndpoint);
        ...
        when(radiusProxyClient.requestProxy(any(), any())).thenAnswer((Answer<RadiusPacket>) invocationOnMock -> {
            IProxyRequestHandler iProxyRequestHandler = invocationOnMock.getArgument(1);
    ...
    public void getRadiusEndpointTest() {
        RadiusEndpoint radiusEndpoint = radiusComponentProxy.getRadiusEndpoint(session, accessRequest);
    ...
    public void getRadiusEndpointTestNull1() {
        when(provider.getRadiusEndpoint(session, AccessRequest.class)).thenReturn(null);
        RadiusEndpoint radiusEndpoint = radiusComponentProxy.getRadiusEndpoint(session, accessRequest);
    ...
    public void getRadiusEndpointTestNull2() {
        when(session.getAllProviders(IRadiusProxyEndpointProvider.class)).thenReturn(new HashSet<>(Collections.emptyList()));
        RadiusEndpoint radiusEndpoint = radiusComponentProxy.getRadiusEndpoint(session, accessRequest);
        assertNull(radiusEndpoint);
    }
    ...
    public void answerHandlerTest() {
        RadiusPacket radiusPacket1 = RadiusPackets.create(realDictionary, ACCESS_ACCEPT, 1, new byte[16]);
        RadiusPacket radiusPacket2 = RadiusPackets.create(realDictionary, ACCESS_REJECT, 1, new byte[16]);
        RadiusPacket radiusPacket = radiusComponentProxy.answerHandler(radiusPacket1, radiusPacket2);
    ...
    public void proxyTest() {
        RadiusPacket radiusPacket = RadiusPackets.create(realDictionary, ACCESS_ACCEPT, 1, new byte[16]);
        when(promise.getNow()).thenReturn(radiusPacket);
        RadiusPacket packet = radiusComponentProxy.proxy(session, accessRequest, radiusPacket);
